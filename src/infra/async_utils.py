...
async def gather_bounded(  # noqa: UP047
    coros: Iterable[Awaitable[T]], limit: int = 5, return_exceptions: bool = False
) -> list[T]:
    """
    ...
    try:
        result = await asyncio.gather(
            *(run(coro) for coro in coro_list), return_exceptions=return_exceptions
        )
...
async def with_timeout(  # noqa: UP047
    coro: Awaitable[T], timeout: float, timeout_msg: str | None = None
) -> T:
    """
